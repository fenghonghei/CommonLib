...
public class Logger {
  ...
  private static void printDeviceInfo(final String filePath) {
    ...
    int versionCode = 0;
    try {
      PackageInfo pi = AppUtil.getApp()
          .getPackageManager()
          .getPackageInfo(AppUtil.getApp().getPackageName(), 0);
      if (pi != null) {
  ...
  public static class Config {
    ...
    private Config() {
      ...
      if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
          && AppUtil.getApp().getExternalCacheDir() != null) {
        mDefaultDir = AppUtil.getApp().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
      } else {
        mDefaultDir = AppUtil.getApp().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
      }
    }
